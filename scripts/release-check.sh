...
FORMULA="$ROOT/Formula/opti-mac.rb"
PACKAGED_FORMULA="$ROOT/packaging/homebrew/opti-mac.rb"
...
[[ -f "$FORMULA" ]] || fail "missing Formula/opti-mac.rb"
[[ -f "$PACKAGED_FORMULA" ]] || fail "missing packaging/homebrew/opti-mac.rb"

root_url="$(extract_value url "$FORMULA")"
packaged_url="$(extract_value url "$PACKAGED_FORMULA")"
[[ -n "$root_url" ]] || fail "Formula/opti-mac.rb is missing url"
...
[[ -n "$root_sha" ]] || fail "Formula/opti-mac.rb is missing sha256"
...
if grep -q 'brew install agungdp150/tap/opti-mac' "$README"; then
